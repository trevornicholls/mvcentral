...
namespace mvCentral.Database
{
  ...
  public class DBLocalMedia : mvCentralDBTable
  {
    ...
    private string _trimmedfullpath;


    [DBRelation(AutoRetrieve = true, Filterable = false)]
    public RelationList<DBLocalMedia, DBTrackInfo> AttachedmvCentral
    {
      get
      {
        if (_attachedmvCentral == null)
        {
          _attachedmvCentral = new RelationList<DBLocalMedia, DBTrackInfo>(this);
        }
        return _attachedmvCentral;
      }
    }
    RelationList<DBLocalMedia, DBTrackInfo> _attachedmvCentral;
    ...
    public override void Delete()
    {
      ...
      deleting = true;
      logger.Info("Removing '{0}' and associated musicvideo.", FullPath);
      //            foreach (DBTrackInfo currMusicVideo in AttachedmvCentral)
      //            {
      //                foreach (DBLocalMedia otherFile in currMusicVideo.LocalMedia)
      //                {
      //                    if (otherFile != this)
    ...
    public static List<DBLocalMedia> GetEntriesByHash(string filehash)
    {
      ...
      return mvCentralCore.DatabaseManager.Get<DBLocalMedia>(criteria);
    }
    ...
    public static List<DBLocalMedia> GetAll(string fullPath, string volumeSerial)
    {
      ...
      List<DBLocalMedia> resultSet = mvCentralCore.DatabaseManager.Get<DBLocalMedia>(criteria);
      return resultSet;
    }

    public static List<DBLocalMedia> GetAll()
    {
      return mvCentralCore.DatabaseManager.Get<DBLocalMedia>(null);
    }
