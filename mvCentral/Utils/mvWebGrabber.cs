...
namespace mvCentral.Utils
{
  class mvWebGrabber
  {
        ...
        public mvWebGrabber(string url) {
            requestUrl = url;
            request = (HttpWebRequest)WebRequest.Create(requestUrl);
        }

        public mvWebGrabber(Uri uri) {
        ...
        public bool NoStatusResponse
        {
          get { return _nostatusresponse; }
          set { _nostatusresponse = value; }
        } private bool _nostatusresponse = false;
        ...
        public bool GetResponse(string apiKey) {
            try {
                ...
                // Only send requests at 1 second or greater intervals
                delay();
                    ...
                    try {
                      ...
                      if (e.Status == WebExceptionStatus.ProtocolError)
                      {
                        using (WebResponse response = e.Response)
                        {
                          ...
                          {
                            lfmStatusResponse = new StreamReader(data).ReadToEnd();
                          }
                          ...
                          switch (statusCode)
                          {
                            ...
                            case HttpStatusCode.BadRequest:
                              XmlDocument lfmStatus = new XmlDocument();
                              lfmStatus.LoadXml(lfmStatusResponse);
                              XmlNode lfmStatusCode = lfmStatus.SelectSingleNode("lfm");
                              XmlNode lfmStatusText = lfmStatusCode.FirstChild;
        ...
        public string GetString() {
            if (response == null && NoStatusResponse)
                return null;
        ...
        private void delay()
        {
          ...
          if (mvWebGrabber.lastCallTime == null)
            mvWebGrabber.lastCallTime = new Nullable<DateTime>(DateTime.Now);

          if (DateTime.Now.Subtract(mvWebGrabber.lastCallTime.Value) > new TimeSpan(0, 0, 1))
            Thread.Sleep(250);
        }
