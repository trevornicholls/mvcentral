...
namespace mvCentral.Player
{
  public class mv_Player
  {
    #region enums
...
/*    public enum MediaType
  ...
  */    
    ...
    private static g_Player.MediaType _currentMedia;
    private static IPlayerFactory _factory= new PlayerFactory();
    ...
    public delegate void StoppedHandler(g_Player.MediaType type, int stoptime, string filename);
    public delegate void EndedHandler(g_Player.MediaType type, string filename);
    public delegate void StartedHandler(g_Player.MediaType type, string filename);
    public delegate void ChangedHandler(g_Player.MediaType type, int stoptime, string filename);
    ...
    private static void ChangeDriveSpeed(string strFile, g_Player.DriveType drivetype)
    {
      if (!driveSpeedLoaded)
      {
        LoadDriveSpeed();
      ...
      try
      {
        ...
        if (rootPath != null)
        {
          if (rootPath.Length > 1)
          {
            ...
            {
              if (drivetype == g_Player.DriveType.CD && _disableCDSpeed[driveindex] == "N")
              {
                speed = _driveSpeedCD[driveindex];
              }
              else if (drivetype == g_Player.DriveType.DVD && _disableDVDSpeed[driveindex] == "N")
              {
    ...
    private static void OnChanged(string newFile)
    {
      ...
      if (!newFile.Equals(CurrentFile))
      {
        ...
        {
            if ((_currentMedia == g_Player.MediaType.TV || _currentMedia == g_Player.MediaType.Video || _currentMedia == g_Player.MediaType.Recording) &&
              (!MediaPortal.Util.Utils.IsVideo(newFile)))
          {
    ...
    private static void OnStarted()
    {
      ...
      {
        //yes, then raise event 
          _currentMedia = g_Player.MediaType.Music;
        if (_player.IsTV)
        {
            _currentMedia = g_Player.MediaType.TV;
          if (!_player.IsTimeShifting)
          {
              _currentMedia = g_Player.MediaType.Recording;
          }
        }
        else if (_player.IsRadio)
        {
          _currentMedia = g_Player.MediaType.Radio;
        }
        else if (_player.HasVideo)
        {
          if (_player.ToString() != "MediaPortal.Player.BassAudioEngine")
          {
            _currentMedia = g_Player.MediaType.Video;
          }
        }
    ...
    private static void doStop(bool keepTimeShifting, bool keepExclusiveModeOn)
    {
      ...
      if (_player != null)
      {
        ...
        if (!String.IsNullOrEmpty(MediaPortal.Util.DaemonTools.GetVirtualDrive()))
            MediaPortal.Util.DaemonTools.UnMount();
      }
    }
...
//            RefreshRateChanger.RefreshRateChangeg_Player.MediaType == RefreshRateChanger.g_Player.MediaType.TV)
        {
//          return true;
        }
        if (_player == null)
    ...
    public static bool IsTVRecording
    {
      ...
      {
//        if (RefreshRateChanger.RefreshRateChangePending &&
//            RefreshRateChanger.RefreshRateChangeg_Player.MediaType == RefreshRateChanger.g_Player.MediaType.Recording)
        {
//          return true;
        }
        if (_player == null)
        ...
        return (_currentMedia == g_Player.MediaType.Recording);
      }
    }
    ...
    {
      return Play(strPath, g_Player.MediaType.Video);
    }
    ...
    {     
      try
      {
        ...
        RefreshRateChanger.AdaptRefreshRate(strPath, RefreshRateChanger.g_Player.MediaType.Video);
        if (RefreshRateChanger.RefreshRateChangePending)
        {
    ...
    public static bool PlayAudioStream(string strURL, bool isMusicVideo)
    {
      try
      {
        ...
        if (!bResult)
        {
          UnableToPlay(strURL, g_Player.MediaType.Unknown);
        }
        return bResult;
      }
      finally
    ...
    {
      return Play(strFile, g_Player.MediaType.Unknown);
    }

    public static bool Play(string strFile, g_Player.MediaType type)
    {
      return Play(strFile, type, (TextReader)null);
    }

    public static bool Play(string strFile, g_Player.MediaType type, string chapters)
    {
    ...
    public static bool Play(string strFile, g_Player.MediaType type, TextReader chapters)
    {
      try
      {
        ...
        bool isImageFile = MediaPortal.Util.VirtualDirectory.IsImageFile(extension);
        if (isImageFile)
        {
            if (!File.Exists(MediaPortal.Util.DaemonTools.GetVirtualDrive() + @"\VIDEO_TS\VIDEO_TS.IFO"))
          {
            _currentFilePlaying = strFile;
            MediaPortal.Ripper.AutoPlay.ExamineCD(MediaPortal.Util.DaemonTools.GetVirtualDrive(), true);
            return true;
          }
        }
        ...
        {
          ChangeDriveSpeed(strFile, g_Player.DriveType.CD);
        }
...
//          type = g_Player.MediaType.Radio;
//        }
        // End of fix
//        else
        {
            if (MediaPortal.Util.Utils.IsVideo(strFile) || MediaPortal.Util.Utils.IsLiveTv(strFile)) //video, tv, rtsp
          {
            if (type == g_Player.MediaType.Unknown)
            {
        ...
        if (_player != null)
        {
          ...
          OnStopped();
          bool doStop = true;
          if (type != g_Player.MediaType.Video && MediaPortal.Util.Utils.IsAudio(strFile))
          {
            if (type == g_Player.MediaType.Unknown)
            {
              type = g_Player.MediaType.Music;
            }
        ...
        {
            if (!MediaPortal.Util.Utils.IsRTSP(strFile) && extension != ".ts") // do not play recorded tv with external player
          {
            using (MediaPortal.Profile.Settings xmlreader = new MediaPortal.Profile.MPSettings())
            {
                  ...
                  (!bInternal && (extension != ".ifo" && extension != ".vob" && !isImageFile))) // external player used
              {
                if (isImageFile)
                {
                    strFile = MediaPortal.Util.DaemonTools.GetVirtualDrive() + @"\VIDEO_TS\VIDEO_TS.IFO";
                }
...
//              _chaptersname = _player.ChaptersName;
    ...
    public static bool IsMusic
    {
      get
      {
        ...
        return (_currentMedia == g_Player.MediaType.Music);
      }
    }
    ...
    public static bool IsVideo
    {
      get
      {
        ...
        if (_currentMedia == g_Player.MediaType.Video)
        {
          return true;
        }
        return false;
...
*/    }
    ...
    public static void UnableToPlay(string FileName, g_Player.MediaType type)
    {
      try
      {
