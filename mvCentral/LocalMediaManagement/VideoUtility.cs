    ...
    public static class VideoUtility {
        ...
        public static bool isSampleFile(FileInfo file) {
            try {
                ...
                Regex expr = new Regex(mvCentralCore.Settings.SampleRegExFilter, RegexOptions.IgnoreCase);
                // Set sample max size in bytes and check the file size
                long sampleMaxSize = mvCentralCore.Settings.MaxSampleFilesize * 1024 * 1024;
                bool match = (file.Length < sampleMaxSize);
                if (match) {
                    // check the filename
                    match = expr.Match(file.Name).Success;
                    if (!match && mvCentralCore.Settings.SampleIncludeFolderName) {
