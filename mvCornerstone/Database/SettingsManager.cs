...
namespace mvCornerstone.Database {
    public abstract class SettingsManager: Dictionary<string, DBSetting> {
        ...
        private Dictionary<PropertyInfo, mvCornerstoneSettingAttribute> attributeLookup;
        ...
        private void BuildPropertyLookup() {
            ...
            attributeLookup = new Dictionary<PropertyInfo, mvCornerstoneSettingAttribute>();

            foreach (PropertyInfo currProperty in GetType().GetProperties()) {
                // make sure this property is intended to be a setting
                object[] attributes = currProperty.GetCustomAttributes(typeof(mvCornerstoneSettingAttribute), true);
                if (attributes.Length == 0)
                    continue;

                // and store it's info for quick access later
                mvCornerstoneSettingAttribute attribute = attributes[0] as mvCornerstoneSettingAttribute;
        ...
        private void UpdateAndSyncSettings() {
            ...
            foreach (PropertyInfo currProperty in propertyLookup.Values) {
                try {
                    ...
                    mvCornerstoneSettingAttribute attribute = attributeLookup[currProperty];
        ...
        private void SyncSetting(PropertyInfo property) {
            mvCornerstoneSettingAttribute attribute = attributeLookup[property];
    ...
    public sealed class mvCornerstoneSettingAttribute : Attribute {
