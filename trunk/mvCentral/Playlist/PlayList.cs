...
//                item.Played = false;
        ...
        public bool Insert(PlayListItem item, int currentItem)
        {
            ...
            logger.Info(string.Format("Playlist: Insert {0} at {1}", item.FileName, Convert.ToString(currentItem)));
        ...
        public bool Insert(PlayListItem item, PlayListItem afterThisItem)
        {
            ...
            for (int i = 0; i < _listPlayListItems.Count; ++i)
            {
                ...
                {
                    logger.Info(string.Format("Playlist: Insert {0} after {1}", item.FileName, afterThisItem.FileName));
        ...
        public int MovePlayListItemUp(int iItem)
        {
            ...
            try
            {
//                logger.Info(string.Format("Moving playlist item {0} up. Old index:{1}, new index{2}", playListItem1.EpisodeName, iItem, iPreviousItem));
            ...
            {
                logger.Info(String.Format("PlayList.MovePlayListItemUp caused an exception: {0}", ex.Message));
                selectedItemIndex = -1;
            }
            finally
            {
        ...
        public int MovePlayListItemDown(int iItem)
        {
            ...
            try
            {
//                logger.Info(string.Format("Moving playlist item {0} down. Old index:{1}, new index{2}", playListItem1.EpisodeName, iItem, iNextItem));
                System.Threading.Monitor.Enter(this);
                _listPlayListItems[iItem] = playListItem2;
            ...
            {
                logger.Info(string.Format("PlayList.MovePlayListItemDown caused an exception: {0}", ex.Message));
                selectedItemIndex = -1;
            }
            finally
            {
