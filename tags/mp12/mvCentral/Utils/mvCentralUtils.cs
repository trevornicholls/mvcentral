...
namespace mvCentral.Utils
{
  public static class mvCentralUtils
  {
    ...
    /// <summary>
    ///  Check to see if the user has setup a noise filter and if so applies to the source text
    /// </summary>
    /// <param name="sourceText"></param>
    /// <returns></returns>
    public static string bioNoiseFilter(string sourceText)
    {
      if (mvCentralCore.Settings.CustomBioRegex.Trim().Length > 0)
      {
        try
        {
          string newResult = System.Text.RegularExpressions.Regex.Replace(sourceText,
                 mvCentralCore.Settings.CustomBioRegex,
                 string.Empty,
                 System.Text.RegularExpressions.RegexOptions.IgnoreCase);
        ...
        {
          logger.DebugException("Custom regex replacement failed", e);
          return sourceText;
        }
      }
      return sourceText;
